...
def client_headers(client):
    resp = client.post("/api/client/session", json={"email": "miniapp-tests@example.com"})
    assert resp.status_code == 200
    return {"X-Client-Token": resp.json()["client_token"]}


@pytest.fixture(scope="module")
def executed_session(client, client_headers):
    resp = client.post(
        "/api/client/demo",
        json={"text": "Monitor inventory and alert suppliers when stock falls below 20%."},
...
def test_demo_result_includes_dynamic_ui_schema(executed_session):
    ui_schema = executed_session["ui_schema"]
    assert ui_schema is not None
    assert ui_schema["app"]["title"]
    assert len(ui_schema["inputs"]) > 0
    assert len(ui_schema["components"]) > 0


def test_novel_domain_still_produces_a_mini_app(client, client_headers):
...
def test_run_single_record_against_workflow(client, client_headers, executed_session):
    session_id = executed_session["session_id"]
    record_id = executed_session["mini_app"]["records"][0]["id"]

    resp = client.post(f"/api/client/demo/{session_id}/records/{record_id}/run", headers=client_headers)
    assert resp.status_code == 200
    execution = resp.json()
...
def test_run_record_on_nonexistent_session_returns_404(client, client_headers):
    resp = client.post("/api/client/demo/does-not-exist/records/some-id/run", headers=client_headers)
    assert resp.status_code == 404


def test_take_action_persists_and_appears_in_log(client, client_headers, executed_session):
    session_id = executed_session["session_id"]
    record_id = executed_session["mini_app"]["records"][0]["id"]

    resp = client.post(
        f"/api/client/demo/{session_id}/records/{record_id}/actions",
        json={"action": "approve"},
        headers=client_headers,
    )
    assert resp.status_code == 200
    ...
    log = client.get(f"/api/client/demo/{session_id}/actions")
    assert log.status_code == 200
    entries = log.json()
    assert any(e["record_id"] == record_id and e["action"] == "approve" for e in entries)


def test_take_invalid_action_is_rejected(client, client_headers, executed_session):
    session_id = executed_session["session_id"]
    record_id = executed_session["mini_app"]["records"][0]["id"]
